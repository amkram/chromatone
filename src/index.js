...
const KEYS_DEMO = (function () {
  ...
  const cfg = {
    ...
    keyboard88:
      ...
      //  '<svg class="piano-keys" version="1.1" xmlns="http://www.w3.org/2000/svg" xmlns:xlink="http://www.w3.org/1999/xlink" viewBox="0 0 1197.8 140.7"><rect rx="1.5" y="20" x="0.5" class="note note-white keyA0"/><rect rx="1.5" y="20" x="23.5" class="note note-white keyB0"/><text x="0.5" y="10" text-anchor="middle">A0</text><rect rx="1.5" y="20" x="20.2" class="note note-black keyA#0"/><rect rx="1.5" y="20" x="46.5" class="note note-white keyC1"/><rect rx="1.5" y="20" x="69.5" class="note note-white keyD1"/><rect rx="1.5" y="20" x="92.5" class="note note-white keyE1"/><rect rx="1.5" y="20" x="115.5" class="note note-white keyF1"/><rect rx="1.5" y="20" x="138.5" class="note note-white keyG1"/><rect rx="1.5" y="20" x="161.5" class="note note-white keyA1"/><rect rx="1.5" y="20" x="184.5" class="note note-white keyB1"/></svg>',
      keyboard49:
  ...
  const degrees = ["P1", "m2", "M2", "m3", "M3", "P4", "TT", "P5", "m6", "M6", "m7", "M7"];
  function getDegreeInScale(rootNote, queryNote) {
    const notesInScale = ["C", "C#", "D", "D#", "E", "F", "F#", "G", "G#", "A", "A#", "B"];
    const rootIndex = notesInScale.indexOf(rootNote);
    const queryIndex = notesInScale.indexOf(queryNote);

    if (rootIndex === -1 || queryIndex === -1) {
      throw new Error("Invalid note name");
    }
    const degreeIdx = (queryIndex - rootIndex + 12) % 12;
    return degrees[degreeIdx];
  }

  function labelKeys(on) {
    
    const svg = document.querySelector(".piano-keys");
    if (on) {
      const notesOn = document.querySelectorAll("[class*=note-on]");
      for (const note of notesOn) {
        const url = "http://www.w3.org/2000/svg";
        let text = document.createElementNS(url, "text");
        console.log(note);
        const w = parseFloat(note.getAttribute("width"));
        const x = parseFloat(note.getAttribute("x")) + w / 2;
        text.setAttribute("x", x);
        text.setAttribute("y", 10);
        text.setAttribute("text-anchor", "middle")
        text.setAttribute("class", "note-label")
        const name = note.classList[note.classList.length - 1].split('note-on-')[1];
        const char1 = document.createElementNS(url, "tspan");
        const char2 = document.createElementNS(url, "tspan");
        char2.setAttribute("dy", "1.2em");
        char1.setAttribute("x", x);
        char2.setAttribute("x", x);
        char1.textContent = name[0];
        char2.textContent = name[1];
        text.appendChild(char1);
        text.appendChild(char2);
  ...
  function detectNote(evt) {
    labelKeys(false);
    console.log(evt)
    var target = evt.target;
    if (target.classList.contains('note')) {
      var noteClass = target.classList[2];
      var noteName = noteClass.slice(3, -1);
      console.log("Note " + noteName + " was pressed.");
      state.key = noteName;
      for (const note of notes) {
        const svgKeys = document
          .querySelectorAll(`[class*=key${CSS.escape(note)}]`)
        const degree = getDegreeInScale(state.key, note);
        for (let i = 0; i < svgKeys.length; i++) {
          ...
          const classesToRemove = Array.from(el.classList)
            .filter(className => className.startsWith('note-on'));
          el.classList.remove(...classesToRemove);
          el.classList.add(`note-on-${degree}`);
        }
      }
    }
    labelKeys(true);
    ...
    document.querySelectorAll('.button-play').forEach((element) => {
      ...
      if (midi !== undefined) {
        Midi.Midi.fromUrl(midi).then((midiJS) => {
          ...
          document.querySelector('.piano-keys').addEventListener('click', detectNote);
